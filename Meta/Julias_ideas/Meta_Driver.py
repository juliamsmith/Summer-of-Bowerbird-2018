...
def BnS(script, JobName, queue='local'):
    to_submit = ("#!/bin/bash" +
                 "\n#SBATCH -J " + JobName + 
    ...
    for i in range(len(dim_vec)):
        ...
        for j in range(len(m_prop_vec)):
            ...
            for k in range(len(RB_time_vec)):
                ...
                #if !is_local: #potentially use this... see about it... or make it truly work either way
                script=writebatchscript(num_sims, in_titles, out_titles, conditions_name)
                BnS(script, conditions_name, queue='local')
